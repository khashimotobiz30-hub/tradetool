...
const Logic = (() => {
  ...
  function computeEntryJudgment(stockData, marketData) {
    ...
    // 上昇トレンド: VWAP上 & MA5上 & (高値切り上げ or 安値切り上げ)
    const isUptrend   = aboveVwap && aboveMa5 && (highsRising || !lowsFalling);
    // 下降トレンド: VWAP下 & MA5下 & (安値切り下げ or 高値切り下げ)
    ...
    if (isUptrend || (aboveVwap && vwapAboveMa5)) {
      // 押し目位置の判断: 現在値がVWAPまたは5MA付近 (乖離が小さい)
      ...
      const stopLoss     = fmt(supportLevel - recentRange * CFG.STOP_RANGE_MULT);
      const target1      = fmt(recentHigh5  + recentRange * CFG.TARGET1_RANGE_MULT);
      const target2      = fmt(recentHigh15 + recentRange * CFG.TARGET2_RANGE_MULT * 0.5);
      const negationPrice = fmt(supportLevel - CFG.BREAKOUT_INVALIDATE_OFFSET);
      ...
      if (isPullback) {
        ...
        mainScenario = `¥${fmt(supportLevel)}±3 接触後に反発ローソクが確認できれば買い。目標①¥${target1} / ②¥${target2}。`;
        negationCond = `¥${negationPrice} を明確に割り込んだらシナリオ無効。見送り継続。`;
      } else if (isBreakout) {
        stateLabel   = 'ブレイク継続待ち';
        entryTrigger = `¥${fmt(recentHigh5)} 超えを維持できるか確認後に追随`;
        situation    = `直近高値 ¥${fmt(recentHigh5)} をブレイク。ブレイク継続か押し返されるかを確認中。`;
        mainScenario = `¥${fmt(recentHigh5)} を割り込まずに推移 (${CFG.BREAKOUT_CONFIRM_BARS}本確認) できれば乗る。目標①¥${target1} / ②¥${target2}。`;
        negationCond = `¥${fmt(recentHigh5 - CFG.BREAKOUT_INVALIDATE_OFFSET)} を割り込んだらダマシ。見送り。`;
      } else {
        // 上昇だが押し目でもブレイクでもない高値圏
        stateLabel   = '上昇中・待機';
        ...
        negationCond = `¥${negationPrice} 割れで上昇トレンド終了とみなす。`;
      }

      const marketNote = buildMarketNote(marketCond, freshness, 'long');
      ...
      return {
        ...
        comment: {
          ...
          mainScenario: mainScenario + (marketNote ? ` ${marketNote}` : ''),
    ...
    if (isDowntrend || (!aboveVwap && !vwapAboveMa5)) {
      ...
      const stopLoss     = fmt(resistLevel + recentRange * CFG.STOP_RANGE_MULT);
      const target1      = fmt(recentLow5  - recentRange * CFG.TARGET1_RANGE_MULT);
      const target2      = fmt(recentLow15 - recentRange * CFG.TARGET2_RANGE_MULT * 0.5);
      const negationPrice = fmt(resistLevel + CFG.BREAKDOWN_INVALIDATE_OFFSET);
      ...
      if (isBounce) {
        ...
        mainScenario = `¥${fmt(resistLevel)}±3 で上値が抑えられたら空売り。目標①¥${target1} / ②¥${target2}。`;
        negationCond = `¥${negationPrice} を上抜けたら戻りシナリオ崩れ。見送り。`;
      } else if (isBreakdown) {
        ...
        mainScenario = `戻り局面で VWAP (¥${fmt(vwap)}) 付近が抵抗になれば空売り。目標①¥${target1}。`;
        negationCond = `¥${negationPrice} 超えで下落シナリオ無効。見送り。`;
      } else {
        stateLabel   = '下落中・待機';
        entryTrigger = `次の戻り (VWAP/5MA) を待つ`;
        situation    = `下降トレンドだが現在値は安値圏。乗りやすい場所ではない。`;
        mainScenario = `VWAP (¥${fmt(vwap)}) 付近まで戻ったら改めて空売り検討。`;
        negationCond = `¥${negationPrice} 超えで下降トレンド終了。`;
      }

      const marketNote = buildMarketNote(marketCond, freshness, 'short');
